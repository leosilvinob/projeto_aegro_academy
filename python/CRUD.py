...
def insert(data):
    ...
    #cursor.execute('select id from t_user order by id DESC LIMIT 1')
    #user_id = cursor.one()
    cursor.execute(f"INSERT INTO t_tweet (author_id, text_tweet , created_at) VALUES ('{author_id}','{text_tweet}', '{created_at}') ON CONFLICT(text_tweet) DO UPDATE SET author_id = EXCLUDED.author_id, created_at = EXCLUDED.created_at  ")
